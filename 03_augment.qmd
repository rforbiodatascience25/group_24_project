...
data_clean_aug <- read_tsv(file = "data/02_dat_clean.tsv.gz", show_col_types = FALSE )  |>
  ...
  mutate(
    ...
    Antigen = case_when(
      Antigen == -1 ~ "Matched",
      Antigen == 0 ~ "1 mismatch",
      Antigen == 1 ~ "2 mismatches",
      Antigen == 2 ~ "3 mismatches"),
    Alel = case_when(
      Alel == -1 ~ "Matched",
      Alel == 0 ~ "1 mismatch",
      Alel == 1 ~ "2 mismatches",
    ...
    HLAgrI = case_when(
      ...
      HLAgrI == 4 ~ "2 MM (allele)",
      HLAgrI == 5 ~ "2MM (allele/antigen)",
      HLAgrI == 7 ~ "Mismatched"),
    Relapse = case_when( 
      Relapse == 0 ~ "No",
      Relapse == 1 ~ "Yes"),
...
data_clean_aug |> write_tsv("data/03_dat_aug.tsv")
```

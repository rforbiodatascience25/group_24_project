...
#Adding the disease row as column names to the table
colnames(Tab_desc_transp) <- Tab_desc$Disease

#Adding a column with rownames to the table
Tab_desc_transp <- as.data.frame(Tab_desc_transp)
Tab_desc_transp <- Tab_desc_transp |> 
  rownames_to_column(var = "Parameter")
Tab_desc_tib <- tibble(Tab_desc_transp)

#Removing disease row, since it is the column names
Tab_desc_tib <- Tab_desc_tib |>
  filter(ALL != "ALL")

#Export table
write_tsv(Tab_desc_tib, file = here("results/Descriptive_table.tsv"))
```
...
#Show table
kable(Tab_desc_tib, row.names = FALSE, format = "html", align = "r")
```
...
Tab_pres <- Tab_desc_tib |>
  filter(Parameter %in% c("N", "Male patients", "Median recipient age", "Relapse", 
                                "Deceased patients",
                                "Median survival time for deceased patients (days)",
                                "Median follow-up time for survived patients (days)"))

#Export table
write_tsv(Tab_pres, file = here("results/Descriptive_table_present.tsv"))
```
...
#Show table
kable(Tab_pres, row.names = FALSE, format = "html", align = "r")
```
